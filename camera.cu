...
namespace rt_in_one_weekend {
    ...
    public:
        ...
        double defocusAngle = 10; // Variation angle of rays through each pixel
        double focusDist = 3.4; // Distance from camera lookFrom point to plane of perfect focus
    ...
    C_D Ray getRay(const int x, const int y, const Camera &camera, curandState *state) {
        ...
        auto rayOrigin = camera.defocusAngle <= 0 ? camera.center : defocusDiskSample(state, camera);
